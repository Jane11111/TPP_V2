...
class Time_Aware_Attention():
    ...
    def time_aware_multihead_attention(self,queries, # batch_size, t_q, type_emb_size
        ...
        Args:
          query_length: 是query的真实长度
          t_query_length: 是填充后的query真实长度
          key_length: 是key的真实长度
          t_keys_length: 是填充后每条key的长度
          queries: A 3d tensor with shape of [N, T_q, C_q].
          queries_length: A 1d tensor with shape of [N].
          keys: A 3d tensor with shape of [N, T_k, C_k].
          keys_length:  A 1d tensor with shape of [N].
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            # w.r.t. different part of the feature
            outputs = tf.matmul(Q_, tf.transpose(K_, [0, 2, 1]),  name ='3')  # (h*N, T_q, T_k)
            #TODO TODO 只用语义上的attention


            # TODO 时间与语义上的attention
