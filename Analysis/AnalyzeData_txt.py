...
if __name__ == "__main__":
    ...
    root = "D://Project/TPP_V2/data/training_testing_data/data_mimic/fold2/"
    sample_count = 0
    ...
    for idx, filename in enumerate(['train.txt','dev.txt','test.txt']):
        ...
        plt.show()
    print(' count: %d, train count : %d, dev count : %d,test count: %d,type_num:%d, max seq len: %d, min seq len: %d,avg seq len: %.2f' % \
          (sample_count, train_test_count[0],train_test_count[1],train_test_count[2],len(type_dic), max_seq_len, min_seq_len, total_seq_len / sample_count))
