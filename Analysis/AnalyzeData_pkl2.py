...
if __name__ == "__main__":
    ...
    root = "D://Data/Hawkes/data_mimic/total/"
    sample_count = 0
    train_test_count = [0,0]
    train_test_seq_len = [0,0]
    total_seq_len = 0
    max_seq_len = 0
    min_seq_len = 1000
    type_dic = {}

    for idx, filename in enumerate(['train.pkl','test.pkl']):
    ...
    print('seq count: %d,event_count: %d, type_num:%d, max seq len: %d, min seq len: %d,avg seq len: %.2f' % \
          (sample_count, sum(train_test_count),len(type_dic), max_seq_len, min_seq_len, total_seq_len / sample_count))
